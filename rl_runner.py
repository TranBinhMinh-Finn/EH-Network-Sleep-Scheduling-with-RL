...
for i in range(num_episodes):
    ...
    for current_round in count():
        logger.debug('TRAIN', f'Round: {current_round}')
        coverage_reward = 1
        ...
        logger.debug('TRAIN', f'EC_CKN k value: {sleep_scheduler.k}')
    ...
    logger.debug('TRAIN', f'Lifetime: {current_round}')
    plotter.plot_durations(episode_lifetimes, reward_sums, epsilons)
    
end_time = time.time()
logger.info('MAIN', f'COMPLETED IN: {end_time - start_time} seconds')
plotter.plot_durations(episode_lifetimes, reward_sums, epsilons, show_result=True)
plt.ioff()
plt.show()
